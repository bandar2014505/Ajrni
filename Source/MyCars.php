...
<?php
     ...
     if(strip_tags($_SERVER['REQUEST_METHOD']=="POST")){
      ...
      if(isset($_POST['save'])){
        ...
        $sql1 = "INSERT INTO cars (idCars,type,name0,model,color,price,file,date0,idCompany) VALUES ('$idCars','$type', '$name0', '$model', '$color', '$price', '$dt', '$date0','$idCompany')";
            $conn->query("set NAMES utf8");
            if ($conn->query($sql1) === TRUE) {

               $Divnote =  $Divnote . '<div class="alert alert-dismissible alert-success">
       ...
       if(isset($_POST['edit'])){
        ...
        $idCompany = $_SESSION['idUser'];
         ...
         if(isset($_FILES['file2'])){
            ...
            $sql1 = "UPDATE cars SET idCars='$idCars', type='$type',name0='$name0', model='$model', color='$color', price='$price', file='$dt' WHERE id='$id'";
            $conn->query("set NAMES utf8");
            if ($conn->query($sql1) === TRUE) {
     ...
     if ($result2->num_rows > 0) {
      while ($row = $result2->fetch_assoc()) {
        ...
        echo '</td>';
                ...
                echo '<td>';
                echo '<img src="imgcar/' . $row["file"] . '.jpg' . '" style="width:80px" alt="لا يوجد صورة" />';
                echo '</td>';
