...
<?php
     ...
     $Divnote = $userId = $dateFrom =  $dateTo = $Status = $Table = $idCars =  $sql1 = "" ;
     $Status = 4;
    $sum = 0;
    ...
    if ($idCars > 0) {
        $sql1 =  $sql1 . "and cars.idCompany='$idCars'";
    }
     ...
     ?>
                    ...
                    <form method="POST" action="<?php echo $_SERVER['PHP_SELF']?>">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-2">
                                <div class="form-group">
                                    <label for="formFile" class="form-label">الشركة</label>
                                    <select class="form-control" id="idCars" name="idCars">
                                        <option value="0">الكل</option>
                                        <?php
                                                include 'includes/serverdb.php';

            $sql = "SELECT id, fristName FROM users where permissions=1  ORDER BY fristName ASC ";
            $conn->query("set NAMES utf8");
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                  ...
                  if($idCars=="$idd") echo 'selected="selected"';
                    echo '>';
                    echo $row["fristName"];
                    echo '</option>';
                }}
