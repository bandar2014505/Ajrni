       ...
       if(isset($_POST['Accept'])){
        ...
        if ($conn->query($sql) === TRUE) {
          ...
          if ($result4->num_rows > 0) {
           ...
           $receiver = $receiver2;
           $text = "تمت قبول طلبك : " . $dataCar . '- المقدم بتاريخ ' . $date0 . '<a href="MyReservations.php"> اضغط هنا لعرض التفاصيل </a>';
         ...
         if(isset($_POST['delete'])){
            ...
            if ($conn->query($sql) === TRUE) {
              ...
              if ($result4->num_rows > 0) {
               ...
               $sender =  $_SESSION['idUser'];
               $receiver = $receiver2;
               $text = "تمت رفض طلبك : " . $dataCar . '- المقدم بتاريخ ' . $date0 . '<a href="MyReservationsPrecedent.php"> اضغط هنا لعرض التفاصيل </a>';
               $time = date('Y-m-d H:i:s'); 
               $sqlsend = "INSERT INTO notifications_es (receiver, message, date0) VALUES ('$receiver', '$text', '$time')";
               $conn->query("set NAMES utf8");
              if ($conn->query($sqlsend) === TRUE) {
...
<body style="background-color:#FFFFFF;">
 ...
 $sql1 = "SELECT reservations.*, cars.type , cars.name0 , cars.model, cars.color, cars.price, users.fristName, 
...
((TO_SECONDS(reservations.dateTo) - TO_SECONDS(reservations.dateFrom))/3600)* cars.price as priceAll
 FROM reservations left JOIN cars on (reservations.idCars = cars.id) left JOIN users on (reservations.userId = users.id)
  where cars.idCompany='$idUser' and reservations.Status=0 ORDER BY reservations.id DESC";
...
$conn->query("set NAMES utf8");
     ...
     if ($result2->num_rows > 0) {
      while ($row = $result2->fetch_assoc()) {
        ...
        echo '.
        <button type="button" class="btn btn-link  pull-left" id="Modal0" data-toggle="modal"
                ...
                <div class="modal-dialog modal-sm">
                    <div class="modal-content">
                        ...
                        <div class="modal-header">
                            <h4 class="modal-title">اعتماد</h4>

                            <button type="button" class="close pull-left" style="margin-left:8px;"
                                data-dismiss="modal">&times;</button>
                        </div>
                        <div class="modal-body">
                            <h4 class="modal-title">تأكيد اعتماد الطلب</h4>
                        </div>
